...
#define FLOG_SET_OUTPUT(_file)  	flog_set_output(_file)
#define FLOG_CLOSE()  				flog_close()
...
void flog(const char *module, int level, const char *format, ...) __attribute__ ((__format__ (__printf__, 3, 4)));
...
//#define __SLOG_FILE__	__basename(__strdupa(__FILE__))
#define __SLOG_FILE__	__FILE__

#ifdef WIN32
#define SLOG(module, level, fmt, ...) \
	do{\
		const char* bname = __SLOG_FILE__;\
		slog(module, level, "[%12s:%4d] "fmt"\r\n", bname, __LINE__, __VA_ARGS__);\
	}while(0)

#define FLOG(module, level, fmt, ...) \
	do{\
		const char* bname = __SLOG_FILE__;\
		flog(module, level, "[%12s:%4d] "fmt"\r\n", bname, __LINE__, __VA_ARGS__);\
	}while(0)
...
#define SLOG_INFO(module, fmt, ...) \
	do{\
		const char* bname = __SLOG_FILE__;\
		slog(module, SL_INFO, "[%12s:%4d] "fmt"\r\n", bname, __LINE__, __VA_ARGS__);\
	}while(0)
...
#define SLOG_TRACE(fmt, ...) \
	do{\
		const char* bname = __SLOG_FILE__;\
		slog(LM_DEFAULT, SL_CRIT, "[%12s:%4d] "fmt"\r\n", bname, __LINE__, __VA_ARGS__);\
	}while(0)

#define SLOG_ASSERT(exp, fmt, ...) \
	do{\
		if(!(exp)){\
			const char* bname = __SLOG_FILE__;\
			slog(LM_DEFAULT, SL_CRIT, "[%12s:%4d] "fmt"\r\n", bname, __LINE__, __VA_ARGS__);\
...
#define FLOG(module, level, fmt, arg...) \
	do{\
		const char* bname = __SLOG_FILE__;\
		flog(module, level, "[%12s:%4d] "fmt"\r\n", bname, __LINE__, ##arg);\
	}while(0)
...
#define SLOG_INFO(module, fmt, arg...) \
	do{\
		const char* bname = __SLOG_FILE__;\
		slog(module, SL_INFO, "[%12s:%4d] "fmt"\r\n", bname, __LINE__, ##arg);\
	}while(0)
...
#define SLOG_CRIT(module, fmt, arg...) \
	do{\
		const char* bname = __SLOG_FILE__;\
		slog(module, SL_CRIT, "[%12s:%4d] "fmt"\r\n", bname, __LINE__, ##arg);\
	}while(0)

#define SLOG_TRACE(fmt, arg...) \
	do{\
		const char* bname = __SLOG_FILE__;\
		slog(LM_DEFAULT, SL_CRIT, "[%12s:%4d] "fmt"\r\n", bname, __LINE__, ##arg);\
	}while(0)
...
#define SLOG_ASSERT(exp, fmt, arg...) \
	do{\
		if(!(exp)){\
			const char* bname = __SLOG_FILE__;\
			slog(LM_DEFAULT, SL_CRIT, "[%12s:%4d] "fmt"\r\n", bname, __LINE__, ##arg);\
