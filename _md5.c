...
static uint8_t *_md5_enc_file(const char *filename)
{
#define HASH_LEN (16)
	...
	int const in_buf_size = sizeof(in_buf);
	struct MD5Context md5;

	if (filename == NULL)
		return NULL;

	if((src_fd = fopen(filename, "r")) == NULL){
		goto hash_file_err1;
	}

	MD5Init(&md5);
	while((count = fread(in_buf, 1, in_buf_size, src_fd)) > 0) {
		MD5Update(&md5, in_buf, count);
	}
	if(count < 0){
		goto hash_file_err2;
	}
...
static uint8_t* _md5_enc_buffer(const void* buffer, int size)
{
#define HASH_LEN (16)
	static uint8_t hash_value[HASH_LEN * 2 + 2] = {""};
	unsigned char in_buf[4096];
...
{
	return (char*)_md5_enc_file(filename);
}

char* MD5Sum_buffer(const void* buffer, int size)
{
	return (char*)_md5_enc_buffer(buffer, size);
}
